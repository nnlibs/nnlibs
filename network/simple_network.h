...
class SimpleNetwork {
 ...
 public:
  SimpleNetwork(int input_size, int hidden_size, int output_size,
    ...
    auto fc2_w =
        std::make_shared<Tensor>(std::vector<int>{hidden_size, output_size});
    ...
    if (use_gpu) {
      ...
      // LinearGPU(nullptr, nullptr, nullptr, hidden_size, output_size); sigmoid
      // = new SigmoidGPU(nullptr, hidden_size);
    } else {
      fc1 = new LinearCPU(input_size, hidden_size, fc1_w, fc1_b);
      relu = new ReluCPU();
      fc2 = new LinearCPU(hidden_size, output_size, fc2_w, fc2_b);
