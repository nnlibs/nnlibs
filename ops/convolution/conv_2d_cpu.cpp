...
#pragma omp parallel for num_threads(in_channel) reduction(+ : sum)
                for (int ic = 0; ic < in_channel; ic++) {
                    float val = 0.0f;
                    for (int kh = 0; kh < kernel_h; kh++) {
                        int w_index = 0;
                        w_data.GetIndexByIndices({oc, ic, kh, 0}, w_index);
                        int in_index = 0;
                        in_data.GetIndexByIndices({0, ic,
                                                   oh * stride + kh - padding,
                                                   ow * stride - padding},
                                                  in_index);
                        for (int kw = 0; kw < kernel_w; kw++) {
                            val +=
                                w_data[w_index + kw] * in_data[in_index + kw];
                        }
                    }
                    sum += val;
                }
                output({0, oc, oh, ow}) = sum; // 0 means batch is 1
            }
        }
    }
        ...
        for (int h = 0; h < out_height; ++h) {
            for (int w = 0; w < out_width; ++w) {
                ...
                for (int c = 0; c < in_channel; ++c) {
                    for (int kh = 0; kh < kernel_h; ++kh) {
                        int grad_input_start_index = 0;
                        grad_input_mut.GetIndexByIndices(
                            {0, c, h * stride + kh - padding,
                             w * stride - padding},
                            grad_input_start_index);
                        int grad_weight_start_index = 0;
                        grad_weights_mut.GetIndexByIndices(
                            {o, c, kh, 0}, grad_weight_start_index);
                        for (int kw = 0; kw < kernel_w; ++kw) {
                            int grad_index = grad_input_start_index + kw;
                            int weight_index = grad_weight_start_index + kw;
                            grad_input_mut[grad_index] +=
                                grad * weights_mut[weight_index];
                            grad_weights_mut[weight_index] +=
                                grad * input_bakup_mut[grad_index];
                        }
                    }
                }
