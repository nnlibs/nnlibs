...
SelfAttentionCPU::SelfAttentionCPU(int input_dim, int head_dim)
    : SelfAttention(input_dim, head_dim) {}
...
SelfAttentionCPU::SelfAttentionCPU(int input_dim, int head_dim,
    ...
    : SelfAttention(input_dim, head_dim, query_weights, key_weights,
                    value_weights, output_weights) {}

std::shared_ptr<Tensor>
SelfAttentionCPU::Forward(std::shared_ptr<Tensor> input) {
    ...
    for (int i = 0; i < seq_len; ++i) {
        for (int j = 0; j < head_dim; ++j) {
            ...
            for (int k = 0; k < input_dim; ++k) {
                q_data({i, j}) += in_data({i, k}) * qw_data({k, j});
            }
        }
    }
    // K = input * key_weights
    for (int i = 0; i < seq_len; ++i) {
        for (int j = 0; j < head_dim; ++j) {
            k_data({i, j}) = 0.0f;
            for (int k = 0; k < input_dim; ++k) {
                k_data({i, j}) += in_data({i, k}) * kw_data({k, j});
            }
        }
    }
    ...
    for (int i = 0; i < seq_len; ++i) {
        for (int j = 0; j < head_dim; ++j) {
            v_data({i, j}) = 0.0f;
            for (int k = 0; k < input_dim; ++k) {
                v_data({i, j}) += in_data({i, k}) * vw_data({k, j});
            }
        }
    }
    ...
    for (int i = 0; i < seq_len; ++i) {
        for (int j = 0; j < seq_len; ++j) {
            ...
            // / sqrt(d_k)
            at_score({i, j}) =
                dot_product / std::sqrt(static_cast<float>(head_dim));
    ...
    std::shared_ptr<Tensor> output =
        std::make_shared<Tensor>(std::vector<int>{seq_len, head_dim});
...
void SelfAttentionCPU::ZeroGrad() {}
