...
std::shared_ptr<Tensor> MultiHeadAttentionCPU::Forward(
  ...
  for (int i = 0; i < num_heads; ++i) {
    ...
    for (int j = 0; j < seq_len; ++j) {
      for (int k = 0; k < head_dim; ++k) {
        concat_outs_data({j, i * head_dim + k}) = self_atte_data({j, k});
      }
    }
  }
...
std::shared_ptr<Tensor> MultiHeadAttentionCPU::Backward(
